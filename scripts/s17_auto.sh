#!/bin/bash
# S17 — Senior wifi off + Family 2대 (S4_5 의 multi-device 버전, 1 cycle)
#
# 두 Family 모니터링 active 상태에서 Senior wifi off → 양쪽 Family PC disconnect →
# grace 4s → ICE restart 시도. 두 Family 의 처리 대칭성 검증.
#
# 사전 조건:
#   - Family A (R3CR700SEKP) + Family C (RFKYA00Y49L) FamilyDetailScreen
#
# 환경 변수:
#   SENIOR_OFF_S    Senior wifi off 시간 (기본 3)
#   OBSERVE_S       wifi 복귀 후 관찰 시간 (기본 25)
#
# 사용법:
#   bash scripts/s17_auto.sh                       # 기본 3s
#   SENIOR_OFF_S=15 bash scripts/s17_auto.sh       # 긴 단절 (networkLost)
...
SENIOR_OFF_S=${SENIOR_OFF_S:-3}
OBSERVE_S=${OBSERVE_S:-25}

LOG_DIR=e:/tmp/s17_auto
mkdir -p "$LOG_DIR"

trap 'echo "[S17] cleanup: Senior Wi-Fi enable + kill logcat"; adb -s '"$SENIOR_DEVICE"' shell svc wifi enable >/dev/null 2>&1; kill $LOGCAT_A_PID $LOGCAT_C_PID $LOGCAT_S_PID 2>/dev/null' EXIT
...
echo "[S17] PID_A=$PID_A PID_C=$PID_C PID_S=$PID_S"
if [ -z "$PID_A" ] || [ -z "$PID_C" ] || [ -z "$PID_S" ]; then
  echo "[S17] ERROR: app not running"
  exit 1
fi

echo "[S17] === START (senior_off=${SENIOR_OFF_S}s, observe=${OBSERVE_S}s) ==="

# Step 0: 양쪽 다이얼로그 dismiss
for DEV in $FAMILY_A $FAMILY_C; do
  DIALOG_DUMP=$(adb -s $DEV shell "uiautomator dump /sdcard/_s17_check.xml >/dev/null 2>&1; cat /sdcard/_s17_check.xml" 2>/dev/null)
  if echo "$DIALOG_DUMP" | grep -q 'content-desc="확인"'; then
    ...
    echo "[S17] $DEV 다이얼로그 dismiss ($CX,$CY)"
...
echo "[S17] CONNECTED A=$CONNECTED_A C=$CONNECTED_C @ $(date +%T.%3N)"
if [ "$CONNECTED_A" -eq 0 ] || [ "$CONNECTED_C" -eq 0 ]; then
  echo "[S17] ERROR: 한쪽 또는 양쪽 connect 실패"
  exit 1
fi
...
# Step 5: Senior wifi off
T_OFF=$(date +%T.%3N)
echo "[S17] Senior Wi-Fi off @ $T_OFF"
adb -s $SENIOR_DEVICE shell svc wifi disable

# Step 6: wifi off 유지
echo "[S17] Senior Wi-Fi off ${SENIOR_OFF_S}s 유지..."
sleep $SENIOR_OFF_S

# Step 7: Senior wifi on
T_ON=$(date +%T.%3N)
echo "[S17] Senior Wi-Fi on @ $T_ON"
adb -s $SENIOR_DEVICE shell svc wifi enable

# Step 8: observation
echo "[S17] Observation ${OBSERVE_S}s..."
sleep $OBSERVE_S
...
echo "  Family C: ice_restored=$ICE_RESTORED_C networkLost=$NETWORK_LOST_C"

# 대칭성 분류
if [ "$NETWORK_LOST_A" -eq 0 ] && [ "$NETWORK_LOST_C" -eq 0 ] && [ "$ICE_RESTORED_A" -gt 0 ] && [ "$ICE_RESTORED_C" -gt 0 ]; then
  echo "[S17] ✅ 대칭 복구 — 양쪽 ice_restored"
elif [ "$NETWORK_LOST_A" -gt 0 ] && [ "$NETWORK_LOST_C" -gt 0 ]; then
  echo "[S17] ✅ 대칭 networkLost 종결 (Senior 긴 단절 정상)"
elif [ "$NETWORK_LOST_A" -eq 0 ] && [ "$NETWORK_LOST_C" -gt 0 ]; then
  echo "[S17] ⚠ 비대칭: A 복구 / C networkLost — Senior 측 peer 처리 race 검토"
elif [ "$NETWORK_LOST_A" -gt 0 ] && [ "$NETWORK_LOST_C" -eq 0 ]; then
  echo "[S17] ⚠ 비대칭: A networkLost / C 복구 — Senior 측 peer 처리 race 검토"
else
  echo "[S17] ⚠ 분류 불명확 — raw log 확인"
fi
