#!/bin/bash
# S13 Sweep — 영상통화 1→2 전이 시 Family Wi-Fi flap (senior_accepted_auto race)
#
# s13_auto.sh 호출 (s1_3_auto.sh 와 다른 race 전용 스크립트).
# 자동탭 (Senior 수락) 직후 즉시 wifi off → senior_accepted_auto signal 이 RTDB write
# 도중 wifi off → wifi 복귀 후 도달 시 upgrade race trigger.
#
# 사전 조건:
#   - Senior CallActivity 가 영상통화 incoming 자동수락 (ADB tap 발화 onTouchEvent 분기)
#   - Family STREAM_VOICE_CALL 음량 mute
#
# 환경 변수:
#   DURATIONS    공백 구분 timing 목록 (기본 "1 2 3 4 5 6 15 70")
#   BETWEEN_S    사이클 간 대기 (기본 15s — 영상통화 종료 + 재발신 안정화)
#
# 사용법:
#   bash scripts/s13_sweep.sh                                # 기본 8 stages
#   DURATIONS="1 4 6" bash scripts/s13_sweep.sh              # race 가장 자주 발생 timing

set +e

# Family 영상통화 음성 mute
FAMILY_DEVICE=R3CR700SEKP
ORIG_VOL=$(adb -s $FAMILY_DEVICE shell media volume --stream 0 --get 2>/dev/null | grep -oE '[0-9]+' | tail -1)
echo "[SWEEP] Family STREAM_VOICE_CALL 원래 음량 = ${ORIG_VOL:-(unknown)}, mute 처리"
adb -s $FAMILY_DEVICE shell media volume --stream 0 --set 0 >/dev/null 2>&1

trap 'echo "[SWEEP] cleanup: Family Wi-Fi enable + 음량 복원"; adb -s R3CR700SEKP shell svc wifi enable >/dev/null 2>&1; adb -s R3CR700SEKP shell media volume --stream 0 --set ${ORIG_VOL:-5} >/dev/null 2>&1' EXIT

DURATIONS=${DURATIONS:-"1 2 3 4 5 6 15 70"}
BETWEEN_S=${BETWEEN_S:-15}

LOG_DIR=e:/tmp/s13_auto
mkdir -p "$LOG_DIR"
SUMMARY="$LOG_DIR/sweep_summary.log"
> "$SUMMARY"

echo "==== S13 SWEEP START (영상통화 + Family Wi-Fi flap) ====" | tee -a "$SUMMARY"
echo "DURATIONS: $DURATIONS" | tee -a "$SUMMARY"
echo "시작 시각: $(date +%T)" | tee -a "$SUMMARY"

LAST_D=""
for d in $DURATIONS; do LAST_D=$d; done

RESULTS=()

for d in $DURATIONS; do
  echo "" | tee -a "$SUMMARY"
  echo "─── 영상통화 + Family Wi-Fi off ${d}s ───" | tee -a "$SUMMARY"

  if [ $d -ge 60 ]; then
    OBS=90
  elif [ $d -ge 15 ]; then
    OBS=40
  else
    OBS=25
  fi

  CYCLE_OUT=$(FAMILY_OFF_S=$d OBSERVE_S=$OBS bash e:/App/Family/scripts/s13_auto.sh 2>&1)
  echo "$CYCLE_OUT" | grep -E "ice_restored:|renegotiate_done:|noAcceptance:|networkLost:|userHangup:|종결 사유:|✅|⚠" | tee -a "$SUMMARY"

  CLASS=$(echo "$CYCLE_OUT" | grep -oE '\[S13\] [✅⚠] .*' | tail -1)
  RESULTS+=("${d}s: $CLASS")

  if [ "$d" != "$LAST_D" ]; then
    echo "[SWEEP] 사이클 간 ${BETWEEN_S}s 대기..." | tee -a "$SUMMARY"
    sleep $BETWEEN_S
  fi
done

echo "" | tee -a "$SUMMARY"
echo "==== S13 SWEEP 완료 ====" | tee -a "$SUMMARY"
echo "종료 시각: $(date +%T)" | tee -a "$SUMMARY"
echo "" | tee -a "$SUMMARY"
echo "결과 요약:" | tee -a "$SUMMARY"
for r in "${RESULTS[@]}"; do
  echo "  $r" | tee -a "$SUMMARY"
done
echo "" | tee -a "$SUMMARY"
echo "Summary: $SUMMARY"
