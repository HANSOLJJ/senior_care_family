...
| **화면** | Senior 카메라 영상 보임 | Senior 카메라 영상 보임 + 수락 대기 UI |
| **오디오 수신** | Senior 마이크 ON (주변 소리 들림) | Senior 마이크 ON (주변 소리 + 벨소리 들림) |
| **오디오 송신** | OFF (무음, Senior 모름) | 수락 전: OFF / 수락 후: ON (양방향) |
| **비디오 송신** | OFF (Family 카메라 안 씀) | 수락 전: OFF / 수락 후: ON (Family 얼굴 보냄) |
...
|  | 모니터링 | 통화 |
|---|---|---|
| **화면** | 슬라이드쇼 그대로 (변화 없음) | 벨소리 + "전화가 왔어요" UI 표시 |
| **카메라** | WebRTC로 Family에 전송 (시니어 모름) | 동일 (WebRTC로 전송) |
| **마이크** | ON (주변 소리 Family에 전달) | 수락 전: ON / 수락 후: ON (양방향) |
| **스피커** | OFF (무음) | 수락 전: 벨소리 / 수락 후: Family 목소리 |
| **수락 방식** | 없음 (시니어 인지 불필요) | 얼굴 감지 / 화면 터치 / 30초 자동 |
| **수락 후 화면** | — | Family 얼굴 전체 화면 (MonitorCallActivity) |
...
├── WebRTC 연결 (모니터링과 동일)
├── Family 화면: Senior 카메라 영상 + 수락 대기 UI
├── Senior: 벨소리 + "전화가 왔어요" UI 표시
└── Family 오디오/비디오 송신: OFF (아직 양방향 아님)

Phase 2: 시니어 수락 (얼굴 감지 / 터치 / 30초 자동)
...
Phase 3: 양방향 영상통화
├── Family 오디오: ON (양방향)
├── Family 비디오: ON (Family 얼굴 Senior에 표시)
├── Senior: MonitorCallActivity (Family 얼굴 전체 화면)
└── → 완전한 양방향 영상통화
```
...
| **실패 시** | 전화 못 받음 | 터치 또는 30초 후 자동 연결 |
...
- [x] 얼굴 감지 → **수락 수단 중 하나** (터치/30초 타임아웃 대체 가능)
- [x] 통화 중 모니터링 동시 → **OK** (다중 peer, 통화 1명 + 모니터링 N명)

## 남은 미결정 사항

- [ ] Senior 영상 UI 디자인 (전체 화면 vs 오버레이)
- [ ] 30초 자동 연결 시간 조정 (30초 vs 1분)
- [ ] 모니터링 기록 저장 여부 (나중에 검토)
